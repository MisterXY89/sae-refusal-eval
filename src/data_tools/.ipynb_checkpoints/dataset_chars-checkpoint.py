...
class EnhancedDatasetAnalyzer:
    def __init__(self, texts: List[str], use_gpu: bool = False, model_name: str = "gpt2"):
        self.texts = texts
    ...
    def context_length(self) -> float:
        ...
        return float(np.mean([len(text.split()) for text in self.texts]))
    ...
    def self_repetition(self, k: int = 3) -> float:
        ...
        return float(np.log(k * sum(ni + 1 for ni in ngram_freqs)))
    ...
    def calculate_perplexity(self, max_length: int = 512) -> Dict[str, float | None]:
        """
        ...
        if not self.model or not self.tokenizer:
            return {"perplexity": 0.0, "perplexity_gap": None}
        ...
        return {"perplexity": ppl_xl, "perplexity_gap": abs(ppl_xl - ppl_base)}
    ...
    def kmeans_clustering(self, n_clusters: int = 8) -> Dict[str, Union[np.ndarray, List]]:
        ...
        # inference
        distances = [
            min(np.linalg.norm(x - mu) for mu in kmeans.cluster_centers_) for x in X.toarray()
        ]
...
if __name__ == "__main__":
    from base_models_refuse.utils import get_harmful_instructions, get_harmless_instructions
    ...
    harmless_metrics = DatasetMetrics(
        context_length=9.733333333333333,
        self_repetition=5.973809611869261,
        ngram_diversity=0.9913793103448276,
        compression_ratio=1.7151394422310757,
        perplexity=31.38776206970215,
        perplexity_gap=18.539030075073242,
        kmeans_clusters={
            "cluster_assignments": np.array(
                [1, 3, 2, 4, 5, 0, 4, 6, 7, 1, 0, 3, 0, 2, 0], dtype=np.int32
            ),
            "distances": [
                0.5796517516395013,
                0.645582224740537,
    ...
    harmful_metrics = DatasetMetrics(
        context_length=11.533333333333333,
        self_repetition=6.161207321695077,
        ngram_diversity=0.8741258741258742,
        compression_ratio=2.117760617760618,
        perplexity=37.02283477783203,
        perplexity_gap=16.631553649902344,
        kmeans_clusters={
            "cluster_assignments": np.array(
                [2, 6, 3, 4, 5, 0, 2, 1, 7, 2, 3, 1, 1, 2, 1], dtype=np.int32
            ),
            "distances": [
                0.6165458850261806,
                0.0,
